...
import com.tabuk.app.databinding.ActivityAddExamBinding;
...
public class AddExamActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_add_exam);
    ...
    public void addExam(View view) {
        ...
        MyExam myLecture = new MyExam(id, courseName, courseDay, courseTime, selectedFaculty, level, lat, lng);

        firestore.collection("exams").document(id).set(myLecture)
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            Toasty.success(AddExamActivity.this, "Exam added").show();
                            finish();

                        } else {
                            String errorMessage = task.getException().getLocalizedMessage();
                            Toasty.error(AddExamActivity.this, errorMessage).show();
                        }
                    }
                });
